class OracleTestController < ApplicationController
  ...
  def find_by_sql_bind_separate_string
    exec_code('find_by_sql bind separate', %{Hugo.find_by_sql("SELECT * FROM Hugo WHERE Name = :name", [ [ActiveRecord::ConnectionAdapters::Column.new(':name', nil, ActiveRecord::Type::Value.new), 'Name 2'] ])})
  end

  def find_by_sql_bind_separate_integer
    exec_code('find_by_sql bind separate', %{Hugo.find_by_sql("SELECT * FROM Hugo WHERE ID = :id", [ [ActiveRecord::ConnectionAdapters::Column.new(':id', nil, ActiveRecord::Type::Value.new), 2] ])})
  end
  ...
  def exec_query_array
    ...
    exec_code('exec_query Array', %{ActiveRecord::Base.connection.exec_query("SELECT * FROM Hugo WHERE Name = :name", 'My own SQL', [ [ActiveRecord::ConnectionAdapters::Column.new(':name', nil, ActiveRecord::Type::Value.new), 'Name 2'] ])})
...
private
  def exec_code(runmode, code)
    ...
    begin
      ActiveRecord::Base.connection.exec_update("DROP TABLE Hugo", 'Mein SQL', [])
    rescue Exception
    end

    ActiveRecord::Base.connection.exec_update("CREATE TABLE Hugo (ID NUMBER(8) PRIMARY KEY, Name VARCHAR2(200))", 'Mein SQL', [])
    ActiveRecord::Base.connection.exec_update("INSERT INTO Hugo SELECT Level, 'Name '||Level FROM DUAL CONNECT BY Level <= 10", 'Mein SQL', [])
