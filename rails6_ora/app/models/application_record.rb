class ApplicationRecord < ActiveRecord::Base
    ...
    while local_sql['?']
      ...
      if bound_value.class == Array                                             # Collection bound for alias
        ...
        bound_value.each_index do |collection_index|
          ...
          local_binds << bind_internal(coll_bind_alias, bound_value[collection_index])
        end
        ...
        local_binds << bind_internal(bind_alias, bound_value)
      end
    end
  ...
  private
  # do internal binding with correct syntax for Rails version
  def self.bind_internal(bind_alias, bind_value)
    if Rails::VERSION::MAJOR > 4
      ActiveRecord::Relation::QueryAttribute.new(bind_alias, bind_value, ActiveRecord::Type::Value.new)
    else
      [ActiveRecord::ConnectionAdapters::Column.new(bind_alias, nil, ActiveRecord::Type::Value.new), bind_value]
    end
  end
